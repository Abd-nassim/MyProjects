<?php 
	...
	while($message=mysqli_fetch_array($resulte)){
		...
		$mess="$message[2]";
		if($message[4]=="T")
			echo "<input class='messages_button' type='button' onclick='showMessage($mess)' value='to: $etudiant[0] $etudiant[1]' title='$message[1]' />";
		else
			echo "<input style='background:#999;' class='messages_button' type='button' onclick='showMessage($mess)' value='to: $etudiant[0] $etudiant[1]' title='$message[1]' />";
	}
